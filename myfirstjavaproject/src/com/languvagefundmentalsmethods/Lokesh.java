package com.languvagefundmentalsmethods;

public class Lokesh {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
      System.out.println("method started");
      Lokesh l1 = new Lokesh();
        l1.DisplayName();
        l1.DisplayAge();
        l1.DisplayStudent();
        l1.DisplayCollege();
        l1.DispalyBranch();
        l1.DispalyGender();
        l1.displayAddress();
        l1.displayMobile() ;
        l1.displayEmail();
      
        l1.displayMarks();
        l1.displayShowName();
        
        l1.displayshowAge();
        l1.dipslayshowStudent();
        l1.displayshowCollege();
        l1.dispalyshowBranch();
        l1.displayshowGender();
        l1.dispalyAddress();
       
        l1.dispalyMobile();
        l1.dispalyEmail();
        l1.showMarks();
        l1.printHello();
        l1.printWelcome();
        l1.printMessege();
        l1.printJava();
        l1.printMCA();
        l1.printCollegeName();
        l1.printStudentDetails();
        l1.printEmployeeDetails();
        l1.printProductDetails();
        l1.printCompanyDetails();
        l1.displayNumber();
        l1.displaySum();
        l1.displayDiffference();
        l1.displayProduct();
        l1.displayDivision();
        l1.displayRemainder();
       
        l1.displaySquare();
        l1.displayCube();
        l1.displayTable();
        l1.displayFactorial();
        l1.showEvenNumbers();
        l1.showOddNumbers();
        l1.showNaturalNumbers();
        l1.showPositiveNumbers();
        l1.showNegativeNumbers();l1.showPrimeNumbers();
        l1.showReversNumbers();
        l1.showPalindrome();
        l1.showFibonacci();
        l1.showMultiplicationTable();
        l1.checkEven();
        l1.checkOdd();
        l1.checkPrime();
        l1.checkPositive();
        l1.checkNegative();
        l1.checkZero();
        l1.checkPalindrome();
        l1.checkLeepYear();
        l1.checkVowel();
        l1.checkConsonant();
        l1.calculateSalary();
        l1.calculateBonus();
        l1.calculateTax();
        l1.calculateInterest();
        l1.calculateArea();
        l1.calculateperimeter();
        l1.calculateSimpleInterest();
       
        l1.calculateCompound();
        l1.calculatePercentage();
        l1.calculateAverage();
        l1.dispayEmplayee(); 
        l1.dispalyManager();
        l1.dispalyCustomer();
        l1.displayBankAccount();
        l1.displayBankAccount();
        l1.AccountBalance();
        l1.DispalyTranction();
        l1. displayProducts() ;
        l1.displayOrder();
        l1.displayBook();
        l1.displayLibrary();
        l1.startCar();
        l1.stopCar();
        l1.startBike();
        l1.openDoor();
        l1.stopBike();
        l1.closeDoor();
        l1.switchOn();
        l1.switchoff();
        l1.login();
        l1.logout();
        l1.lokesh();
        l1.santhosh();
        l1.sandhya();
        l1.gopi();
        l1.Ashok();
        l1.bhargav();
        l1.AdiNarayana();
        l1.Raju();
        l1.NagaSai();
        
       
        
	}
		void DisplayName() {
			System.out.println("method 1 ");
			
		}
		
		void DisplayAge() {
			System.out.println("method 2");
		}
		void DisplayStudent() {
			System.out.println("methid 3");
		}
		void DisplayCollege() {
			System.out.println("method 4");
		}
		
		void DispalyBranch()
		{
			System.out.println("method 5");
			}
		void DispalyGender() {
			System.out.println("method 6");
		}
		void displayAddress() {
			System.out.println("method 7");
		}
		
		void displayMobile() {
			System.out.println("method 8");
		}
		
		void displayEmail()
		{
			System.out.println("method 9");
		}
		void displayMarks() {
			System.out.println("method 10");
		}
		void displayShowName() {
			System.out.println("method 11");
		}
		void displayshowAge() {
			System.out.println("method 12");
		}
		void dipslayshowStudent() {
			System.out.println("method 13");
		}
		void displayshowCollege() {
			System.out.println("method 14");
		}
		void dispalyshowBranch() {
			System.out.println("method 15");
		}
		void displayshowGender() {
			System.out.println("methid 16");
		}
		void dispalyAddress() {
			System.out.println("method 17");
		}
		void dispalyMobile() {
			System.out.println("method 18");
		}
		void dispalyEmail() {
			System.out.println("method 19");
		}
		void showMarks() {
			System.out.println("method 20");
		}
		void printHello() {
			System.out.println("method 21");
		}
		void printWelcome() {
			System.out.println("method 22");
		}
		void printMessege() {
			System.out.println("method 23");
		}
		void printJava() {
			System.out.println("method 24");
		}
		void printMCA() {
			System.out.println("method 25");
		}
		void printCollegeName() {
			System.out.println("method 26");
		}
		void printStudentDetails() {
			System.out.println("method 27");
		}
		void printEmployeeDetails() {
			System.out.println("method 28");
		}
		void printProductDetails() {
			System.out.println("method 29");
			
		}
		void printCompanyDetails() {
			System.out.println("method 30");
		}
		void displayNumber() {
			System.out.println("method 31");
		}
		void displaySum() {
			System.out.println("method 32");
		}
		void displayDiffference() {
			System.out.println("method 33");
		}
		void displayProduct() {
			System.out.println("method 34");
		}
		 void displayDivision() {
			 System.out.println("method 35");
		 }
		 void displayRemainder() {
			 System.out.println("method 36");
		 }
		 void displaySquare()
		 {
			 System.out.println("method 37");
			 }
		 void displayCube() {
			 System.out.println("method 38");
		 }
		 void displayTable() {
			 System.out.println("method 39");
		 }
		 void displayFactorial() {
			 System.out.println("method 40");
		 }
		 void showEvenNumbers() {
			 System.out.println("method 41");
		 }
		 void showOddNumbers() {
			 System.out.println("method 42");
		 }
		 void showNaturalNumbers() {
			 System.out.println("method 43");
		 }
		 void showPositiveNumbers() {
			 System.out.println("method 44");
		 }
		 void showNegativeNumbers() {
			 System.out.println("method 45");
		 }
		 void showPrimeNumbers() {
			 System.out.println("method 46");
		 }
		 void showReversNumbers() {
			 System.out.println("method 47");
		 }
		 void showPalindrome() {
			 System.out.println("method 48");
		 }
		 void showFibonacci() {
			 System.out.println("method 49");
		 }
		 void showMultiplicationTable() {
			 System.out.println("method 50");
		 }
		 void checkEven() {
			 System.out.println("method 51");
			 
		 }
		
		 
		 void checkOdd() {
			 System.out.println("method 52");
		 }
		 void checkPrime() {
			 System.out.println("method 53");
		 }
		 void checkPositive() {
			 System.out.println("method 54");
		 }
		 void checkNegative() {
			 System.out.println("method 55");	 
		 }
		 void checkZero() {
			 System.out.println("method 56");
		 }
		 void checkPalindrome() {
			 System.out.println("method 57");
		 }
		 void checkLeepYear() {
			 System.out.println("method 58");
		 }
		 void checkVowel() {
			 System.out.println("method 59");
		 }
		 void checkConsonant() {
			 System.out.println("method 60");
		 }
		 void calculateSalary() {
			 System.out.println("method 61");
 
		 }
		 void calculateBonus() {
			 System.out.println("method 62");

		 }
		 void calculateTax() {
			 System.out.println("method 63");
		 }
		 void calculateInterest() {
			 System.out.println("method 64");
		 }
		 void calculateArea() {
			 System.out.println("method 65");
		 }
		 void calculateperimeter() {
			 System.out.println("method 66");
		 }
		 void calculateSimpleInterest() {
			 System.out.println("method 67");
		 }
		 void calculateCompound() {
			 System.out.println("method 68");
		 }
		 void calculatePercentage() {
			 System.out.println("method 69");
		 }
		 void calculateAverage() {
			 System.out.println("method 70");
		 }
		 void dispayEmplayee() {
			 System.out.println("method 71");
		 }
		 void dispalyManager() {
			 System.out.println("method 72");
		 }
		 void dispalyCustomer() {
			 System.out.println("method 73");
		 }
		 void displayBankAccount() {
			 System.out.println("method 74");
		 }
		 void AccountBalance() {
			 System.out.println("method 75");
		 }
		 void DispalyTranction() {
			 System.out.println("method 76");
		 }
		 void displayProducts() {
			 System.out.println("method 77");
		 }
		 void displayOrder() {
			 System.out.println("method 78");
		 }
		 void displayBook() {
			 System.out.println("method 79");
		 }
		 void displayLibrary() {
			 System.out.println("method 80");
		 }
		 void startCar() {
			 System.out.println("method 81");
		 }
		 void stopCar() {
			 System.out.println("method 82");
		 }
		 void startBike() {
			 System.out.println("method 83");
		 }
		 void openDoor() {
			 System.out.println("method 84");
		 }
		 void stopBike() {
			 System.out.println("method 85");
		 }
		 void closeDoor() {
			 System.out.println("method 86");
		 }
		 void switchOn() {
			 System.out.println("method 87");
		 
		 }
		 void login() {
			 System.out.println("method 88");
		 }
		 void switchoff() {
			 System.out.println("method 89");
		 }
		 void logout() {
			 System.out.println("method 90");
		 }
		 void lokesh() {
			 System.out.println("method 91");
		 }
		 void santhosh() {
			 System.out.println("method 92");
		 }
		 void sandhya() {
			 System.out.println("method 93");
		 }
		 void gopi() {
			 System.out.println("method 94");
		 }
		 void bhargav() {
			 System.out.println("method 95");
		 }
		 void Ashok() {
			 System.out.println("method 96");
		 }
		 void AdiNarayana() {
			 System.out.println("method 97");
		 }
		 void Raju() {
			 System.out.println("method 98");
		 }
		 void NagaSai() {
			 System.out.println("method 99");
			 
			 
			 System.out.println("METHOD ENDED");
		 }

}


